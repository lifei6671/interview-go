...
最后输出结果是第一个循环结果不固定，主要看协程的调度时间，如果调度时间很慢，可能是全部是`10`。第二个循环会依次输出`0-9`。
...
`make` 在初始化切片时指定了长度，所以追加数据时会从`len(s) - 1` 位置开始填充数据。
...
在 golang 语言中，`Stduent` 和 `*Stduent` 是两种类型，第一个是表示 `Stduent` 本身，第二个是指向 `Stduent` 的指针。
